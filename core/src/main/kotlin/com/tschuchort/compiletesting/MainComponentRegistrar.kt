...
internal class MainComponentRegistrar : ComponentRegistrar {
  ...
  override fun registerProjectComponents(
    ...
    if (threadLocalParameters.get() == null) {
      System.err.println(
          "WARNING: MainComponentRegistrar::registerProjectComponents accessed before thread local parameters have been set")
      return
    }
  ...
  companion object {
    /**
     * This compiler plugin is instantiated by K2JVMCompiler using a service locator. So we can't
     * just pass parameters to it easily. Instead, we need to use a thread-local global variable to
     * pass any parameters that change between compilations
     */
