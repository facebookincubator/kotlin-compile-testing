...
class KotlinCompilation : AbstractKotlinCompilation<K2JVMCompilerArguments>() {
  ...
  @Deprecated("Removed in latest Kotlin version") var skipRuntimeVersionCheck: Boolean? = null
  ...
  inner class Result(
    ...
    val classLoader =
        URLClassLoader(
            // Include the original classpaths and the output directory to be able to load classes
            // from dependencies.
  ...
  private fun commonK2JVMArgs() =
      commonArguments(K2JVMCompilerArguments()) { args ->
        ...
        if (strictJavaNullabilityAssertions != null)
            args.trySetDeprecatedOption(
                "strictJavaNullabilityAssertions", strictJavaNullabilityAssertions)
        ...
        if (constructorCallNormalizationMode != null)
            args.trySetDeprecatedOption(
                "constructorCallNormalizationMode", constructorCallNormalizationMode)
        ...
        if (noExceptionOnExplicitEqualsForBoxedNull != null)
            args.trySetDeprecatedOption(
                "noExceptionOnExplicitEqualsForBoxedNull", noExceptionOnExplicitEqualsForBoxedNull)
  ...
  private fun stubsAndApt(sourceFiles: List<File>): ExitCode {
    ...
    val kaptOptions =
        KaptOptions.Builder().also {
          ...
          if (verbose) it.flags.addAll(KaptFlag.MAP_DIAGNOSTIC_LOCATIONS, KaptFlag.VERBOSE)
        }
    ...
    /**
     * The main compiler plugin (MainComponentRegistrar) is instantiated by K2JVMCompiler using a
     * service locator. So we can't just pass parameters to it easily. Instead, we need to use a
     * thread-local global variable to pass any parameters that change between compilations
     */
  ...
  /**
   * Base javac arguments that only depend on the the arguments given by the user Depending on which
      ...
      mutableListOf<String>().apply {
        ...
        addAll(
            ...
            (commonClasspaths() + classesDir).joinToString(
                File.pathSeparator, transform = File::getAbsolutePath))
      }

  /** Performs the 4th compilation step to compile Java source files */
  private fun compileJava(sourceFiles: List<File>): ExitCode {
    val javaSources =
        (sourceFiles + kaptSourceDir.listFilesRecursively()).filterNot<File>(
            File::hasKotlinFileExtension)
