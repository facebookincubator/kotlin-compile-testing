...
abstract class AbstractKotlinCompilation<A : CommonCompilerArguments> internal constructor() {
  ...
  protected inline fun <reified T> CommonCompilerArguments.trySetDeprecatedOption(
      optionSimpleName: String,
      value: T
  ) {
    try {
      this.javaClass
          .getMethod(JvmAbi.setterName(optionSimpleName), T::class.java)
          .invoke(this, value)
  ...
  protected fun commonArguments(args: A, configuration: (args: A) -> Unit): A {
    ...
    if (languageVersion != null) args.languageVersion = this.languageVersion
     ...
     * locator, so we need to apply the same trick as with [ComponentRegistrar]s: We put our own
     * static [CommandLineProcessor] on the classpath which in turn calls the user's dynamic
  ...
  protected fun compileKotlin(
    ...
    MainComponentRegistrar.threadLocalParameters.set(
        MainComponentRegistrar.ThreadLocalParameters(
            listOf(), KaptOptions.Builder(), compilerPlugins))

    // in this step also include source files generated by kapt in the previous step
    val args =
        arguments.also { args ->
          args.freeArgs =
              sources.map(File::getAbsolutePath).distinct() +
                  if (sources.none(File::hasKotlinFileExtension)) {
                    ...
                    listOf(
                        SourceFile.new("emptyKotlinFile.kt", "")
                            .writeIfNeeded(sourcesDir)
                            .absolutePath)
                  } else {
                    emptyList()
                  }
          args.pluginClasspaths =
              (args.pluginClasspaths
                  ?: emptyArray()) +
                  /**
                   * The resources path contains the MainComponentRegistrar and
                   * MainCommandLineProcessor which will be found by the Kotlin compiler's service
                   * loader. We add it only when the user has actually given us ComponentRegistrar
                   * instances to be loaded by the MainComponentRegistrar because the experimental
                   * K2 compiler doesn't support plugins yet. This way, users of K2 can prevent
                   * MainComponentRegistrar from being loaded and crashing K2 by setting both
                   * [compilerPlugins] and [commandLineProcessors] to the emptyList.
                   */
    ...
    return this::class
        ...
        .find { resourcesPath ->
          ServiceLoaderLite.findImplementations(
                  ComponentRegistrar::class.java, listOf(resourcesPath.toFile()))
  ...
  protected fun searchSystemOutForKnownErrors(compilerSystemOut: String) {
    if (compilerSystemOut.contains(
        "No enum constant com.sun.tools.javac.main.Option.BOOT_CLASS_PATH")) {
      warn(
  ...
  protected fun log(s: String) {
    if (verbose) internalMessageStream.println("logging: $s")
  }
  ...
  internal val internalMessageStreamAccess: PrintStream
    get() = internalMessageStream
}
